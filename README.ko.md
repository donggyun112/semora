...
**내구 실행(durable execution)만으로는 왜 부족한가?** 내구 실행은 단계를 재생한다. 하지만 어떤 이펙트가 이미 바깥으로 나갔는지는 말해주지 않고, 멈춰 있던 호출이 돌아왔을 때 정책이 다시 판단하게 만들지도 않는다. Semora가 맡는 게 정확히 이 둘이다. 시작만 하고 결과를 보고하지 않은 툴 호출은 호스트가 외부 상태를 대조하기 전까지 `Indeterminate`로 남는다. `ConfirmedEffect`는 확인된 결과를 기록하고, `RetryEffect`는 버전에 묶인 재시도 한 번만 허용한다. 그리고 사람의 승인은 새 정책 판단의 입력일 뿐, 판단 그 자체가 아니다.
...
- **이펙트 기록:** 완료된 툴 호출은 기록해둔 결과를 그대로 재생한다. 시작했지만 보고되지 않은 이펙트는 기본적으로 `Indeterminate`다. `unresolved_effects()`로 호출과 버전을 읽고, `resolve_effect()`로 외부에서 확인한 완료 또는 해당 이펙트 한 건의 재시도 결정을 멱등하게 기록한다.
...
원장의 툴 호출 키는 하나의 실행 범위 안에서만 유효하다. 여러 실행이나 브랜치에 걸쳐 중복을 제거해야 하는 비즈니스 오퍼레이션은 호스트가 소유하는 안정적인 키를 따로 마련해야 한다. `resolve_effect(..., workers_stopped=True)`는 이전 워커와 아직 끝나지 않은 외부 요청까지 대조했다는 호스트의 선언이다. 리스는 원장 쓰기만 막을 뿐 외부 API 요청을 취소하지 못한다. `retry_running=True`는 기존의 실행 전체 재시도 옵션으로 남는다. post-tool 훅은 완료 표시를 남기기 전에 크래시가 나면 최소 한 번(at least once) 실행되므로, 외부 효과가 있다면 멱등하게 작성해야 한다.
