...
에이전트 루프, 메시지, 모델, 툴은 Pydantic AI가 맡는다. Semora는 그 위에 call-id 원장(ledger), 워커 리스와 펜싱, 승인 대기의 내구성 있는 중단, 재개 시점의 정책 재검증, 그리고 조합 가능한 일곱 개의 제어 지점을 얹는다. 평범한 Pydantic AI 에이전트를 그대로 써도 되고, Semora가 선택적으로 제공하는 클래스 기반 에이전트 인터페이스를 써도 된다.
...
| 배포 | 임포트 | 책임 |
|---|---|---|
| `semora` | `semora` | 이펙트 능력, 제어, 런타임, 클래스 에이전트, 트랜스크립트와 디스패치 |
