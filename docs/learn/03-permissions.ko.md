...
```python
agent = Agent(FunctionModel(scripted), tools=[...])
runtime = AgentRuntime(MemorySteps())
controls = ControlPlane(pre_tool_use=Permissions(allow({"permission": "read"})))

# 비동기 demo() 안에서 runtime.run(...)
# 승인 답이 오면 runtime.resume(...)
```

`Agent`는 Pydantic AI의 네이티브 클래스이고 여러 요청에서 재사용할 수 있다.
`AgentRuntime`에 전달하는 `"perm-1"`이 시작·승인·복구가 같은 작업임을 찾는 branch 좌표다.
...
read_tool = Tool(read, metadata={"permission": "read"})
```

여기서 모델이 채우는 작업 인자는 `path`다. `metadata`는 개발자가 Pydantic AI의
`Tool` 정의에 붙인 선언이다.
...
`demo()`는 정책을 명시적인 `ControlPlane`으로 런타임에 전달한다.
...
4. `ControlPlane.pre_tool_use`에 연결해서 도구 실행 전에 판단한다.
